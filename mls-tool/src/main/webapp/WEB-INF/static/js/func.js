...
function clearSampleData() {
  ...
  angular.forEach(
      mls_meta[scope.selectedResource][scope.selectedClass]["data"], function(
          value, key) {
        value.sample = "";
      });
}
...
function setSampleData(num) {
  ...
  angular.forEach(
      lcolJSON[scope.selectedResource][scope.selectedClass]["data"], function(
          value, key) {
        if (sampleData && sampleData[num]) {
          if (value.js) {
            eval(value.js);
            var map = jsonToMap(sampleData[num]);
            var result = fun(value.mlsCols, map);
            value.sample = result;
          } else {
            var mlsCol = value.mlsCols;
            value.sample = sampleData[num][mlsCol];
          }
        } else {
          value.sample = "";
        }
